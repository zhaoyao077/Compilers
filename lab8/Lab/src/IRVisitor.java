...
public class IRVisitor extends SysYParserBaseVisitor<LLVMValueRef> {
    ...
    private final Map<String,String> functionRetTypeMap = new HashMap<>();
    private final Map<String,Integer> globalVarValueMap = new HashMap<>();
    private final Map<String,LLVMValueRef[]> globalArrayMap = new HashMap<>();
    private LLVMValueRef curBlock;
    private Deque<WhileScope> loopStack = new ArrayDeque<WhileScope>();
    private WhileScope currentLoop;
    ...
    private final LLVMTypeRef i32PointerType = LLVMPointerType( i32Type,0 );
    ...
    public LLVMValueRef visitFuncDef(SysYParser.FuncDefContext ctx) {
        ...
        for (int i = 0; i < argumentCount; i++) {
            if(ctx.funcFParams().funcFParam(i).getText().contains("[")){
                argumentTypes.put(i, i32PointerType);
            } else {
        ...
        for (int i = 0; i < argumentCount; i++) {
            ...
            if(FParam.getText().contains("[")){
                varPointer = LLVMBuildAlloca(builder, i32PointerType, varName);
            } else {
    ...
    private int resolveConstExp(SysYParser.ConstExpContext ctx){
        ...
        if(currentScope == globalScope){//global var
            for (SysYParser.ConstDefContext varDefContext : ctx.constDef()){
                ...
                if(varDefContext.L_BRACKT(0) == null) {//int a = 1;
                    ...
                    //TODO:elemCount
                    int elemCount = resolveConstExp(varDefContext.constExp(0));
                    LLVMTypeRef arrayType = LLVMArrayType(i32Type,elemCount);
                    globalVar = LLVMAddGlobal(module,arrayType,varName);
                    LLVMValueRef []initArray = new LLVMValueRef[elemCount];
                    for (int i = 0; i < elemCount; i++) {
    ...
    public LLVMValueRef visitLtCond(SysYParser.LtCondContext ctx) {
        ...
        LLVMValueRef condition = LLVMBuildICmp(builder,op,visit(ctx.cond(0)),visit(ctx.cond(1)),"tmp_");
        LLVMValueRef zext = LLVMBuildZExt(builder, condition, i32Type, "tmp_");
        return zext;
    }
    ...
    public LLVMValueRef visitConditionStmt(SysYParser.ConditionStmtContext ctx) {
        LLVMValueRef cond = LLVMBuildICmp(builder,LLVMIntNE,zero,LLVMBuildZExt(builder, visit(ctx.cond()), i32Type, "tmp_"),"temp");
    ...
    public LLVMValueRef visitWhileStmt(SysYParser.WhileStmtContext ctx) {
        WhileScope whileScope = new WhileScope("while",currentScope);

        pushScope(whileScope);

        LLVMBasicBlockRef whileCondtion = LLVMAppendBasicBlock(curBlock,"whileCondition");
        LLVMBasicBlockRef whileBody = LLVMAppendBasicBlock(curBlock,"whileBody");
        LLVMBasicBlockRef whileEnd = LLVMAppendBasicBlock(curBlock,"entry");

        whileScope.setBlock("cond", whileCondtion);
        whileScope.setBlock("end", whileEnd);

        //Append code in while condition
        LLVMBuildBr(builder, whileCondtion);
        LLVMPositionBuilderAtEnd(builder, whileCondtion);
        LLVMValueRef value = LLVMBuildICmp(builder,LLVMIntNE,zero,LLVMBuildZExt(builder, visit(ctx.cond()), i32Type, "tmp_"),"tmp_");

        LLVMBuildCondBr(builder, value, whileBody, whileEnd);

        //Append code in while body
        LLVMPositionBuilderAtEnd(builder, whileBody);
        visit(ctx.stmt());
        LLVMBuildBr(builder, whileCondtion);

        //Append code after while
    ...
    private void pushScope(WhileScope scope){
